---

# ── IDENTITY ────────────────────────────────────────────────────────────────
type: person
native_name: Zagoste Paroska
lusitanized_name:
aliases:
  - Paroska
summary: Dripstanian admiral who sat on the general government of the Federated Provinces of Galil during the Liberal Revolts. His rivalry with Kaichet Satratonie over the Countess of Jafnidia supplied the motive for the defection that ended the republic, and he was executed with Versij in 1844.

# ── DEMOGRAPHICS ────────────────────────────────────────────────────────────
sex: Male
ethnicity: West Gaiyanese
religion: "[[Reformed Armotism]]"
citizenship:
  - "[[Dripstanian Empire]]"
nationality:
  - "[[Susia|Susian]]"
  - "[[Dripstanian Empire|Dripstanian]]"
enhanced: false

# ── BIRTH AND DEATH ─────────────────────────────────────────────────────────
birth:
  year: 1796
  city: "[[Duolij]]"
  state: "[[Postia]]"
  country: "[[Dripstanian Empire]]"
death:
  year: 1844
  city: "[[Neoveli]]"
  state: "[[Dripia]]"
  country: "[[Dripstanian Empire]]"
  cause: Hanging
# ── FAMILY ──────────────────────────────────────────────────────────────────
relations:
  - person: 
    relation: 
    notes: 

# ── EDUCATION ───────────────────────────────────────────────────────────────
education:
  - degree:
    institution: "[[Susian Naval Academy]]"
    year:
  - 

# ── CAREER ──────────────────────────────────────────────────────────────────
occupation:
  - title: Naval Officer
    start_year:
    end_year:
  - title: Politician
    start_year: 1840
    end_year: 1844
military_service:
  - allegiance: "[[Dripstanian Empire]]"
    branch: Navy
    rank: Admiral
    start_year:
    end_year:
    conflicts:
      - 
    notes:
  - allegiance: "[[Federated Provinces of Galil]]"
    branch: Navy
    rank: Admiral
    start_year: 1840
    end_year: 1844
    conflicts:
      - "[[Liberal Revolts]]"
    notes:
political_alignment:
  - "[[List of political positions#Republicanism]]"
party:
parties:
  - 
organizations:
  - "[[Federated Provinces of Galil]]"

# ── OFFICES ─────────────────────────────────────────────────────────────────
offices:
  - title: Member of the General Government of the Federated Provinces of Galil
    employer:
    start_year: 1840
    end_year: 1844
    appointer: General Government of the Federated Provinces of Galil
    parties:
      - Independent
    notes:

# ── WRITTEN WORKS ───────────────────────────────────────────────────────────
written_works:
  - title:
    publication_year:
    genre:
    notes:

# ── CRIMINAL RECORD ─────────────────────────────────────────────────────────
criminal_charges:
  - charge: Treason
    counts: 22
    charged_year: 1844
    plea: Not Guilty
    verdict: Guilty
    verdict_year: 1844
    sentence: Death
    served:
    in_absentia: false
    notes:

# ── ASSOCIATIONS & LEGACY ───────────────────────────────────────────────────
known_for:
  - item: "[[Liberal Revolts]]"
    notes: Member of the general government of the Federated Provinces of Galil.
  - item: "[[Federated Provinces of Galil]]"
    notes:
  - item: "[[Countess of Jafnidia]]"
    notes: The affair that supplied Satratonie's motive for defecting.
  - item: Martyr of the Liberal Revolts
    notes: Executed with Versij in 1844.
awards:
  - title:
    awarded_year:
    posthumous: false
    granted_by:
    country:
    notes:
era:
  - late-imperial
  - high-imperial
  - imperial-era
  - liberal-revolts

# ── META ────────────────────────────────────────────────────────────────────
tags:
  - military
  - politics
  - history
meta:
  stub: false
  verified: false
  image:
---

**Zagoste Paroska** (1796–1844) was a [[Dripstanian Empire|Dripstanian]] naval officer who rose to admiral in the imperial navy and then sat on the general government of the [[Federated Provinces of Galil]] during the [[Liberal Revolts]]. He was publicly executed alongside [[Armadesh Versij]] in 1844 and is remembered in [[Susia]] as a martyr of liberty.

## Early Life

Paroska was born in 1796 in [[Duolij]], in the imperial territory that later became the state of [[Postia]]. He came from a prestigious family. He trained at the [[Susian Naval Academy]].

%% TODO: the family Paroska came from is described as prestigious in [[Cericeiro honor code]] but is not named anywhere in the vault. No house, no relations, no holdings established. %%

## Naval career

Paroska served in the navy of the [[Dripstanian Empire]] and reached the rank of admiral before the revolt.

%% TODO: commands, campaigns, promotion dates, and the point at which he broke with the empire are not established. %%

## Liberal Revolts

> *Main article: [[Liberal Revolts]]*

### The general government

In 1840 a general assembly in [[Neoveli]] proclaimed the Federated Provinces of Galil, joined by Duolij, [[New Kentu]], and [[Niqueparj]]. Paroska sided with the republic and sat on its general government alongside Versij and General [[Kaichet Satratonie]]. The body was constituted as three equals, and its authority consolidated around Versij as the war continued.

### Rivalry with Satratonie

Paroska and Satratonie were rivals throughout the revolt. Paroska had conducted an affair with the [[Countess of Jafnidia]], a woman Satratonie wished to marry, and Satratonie repeatedly sought to duel him over it.

The challenges went unanswered. Formal dueling in this period was a western institution conducted between social equals, and Satratonie, who had risen through the imperial officer corps by merit rather than birth, was not accepted as one by the order Paroska belonged to.

Satratonie concluded that wealth, status, and the death of his rival together would secure the marriage. On that expectation he accepted [[Jartes II]]'s offer of land, money, and a royal pardon in exchange for changing sides.

### Defeat and execution

Satratonie defected. The fort at [[Jaffnidia]] fell, the republic was split in two, and imperial troops entered Neoveli in 1844. Paroska, Versij, and the rest of the republican leadership were publicly executed. Paroska was convicted on 22 counts of treason, having pleaded not guilty, and was hanged.

## Legacy

Paroska and Versij are remembered in Susia as the martyrs of the Liberal Revolts, and Jartes II as the emperor who killed them. Versij's legacy is doctrinal and is carried by [[Versij Day]], [[Versijian Square]], and the philosophical tradition that bears his name. Paroska's rests on the revolt itself and on the personal quarrel that ended it. [[Jaffnidia]], where the defection began, holds a museum dedicated to the revolts.

## See also

- [[Liberal Revolts]]
- [[Federated Provinces of Galil]]
- [[Armadesh Versij]]
- [[Kaichet Satratonie]]
- [[Countess of Jafnidia]]
- [[Jartes II]]
- [[Jaffnidia]]
- [[Cericeiro honor code]]
- [[Susian Naval Academy]]
- [[Duolij]]
- [[Neoveli]]

%% FLAGS:

- Article migrated from the pre-migration person schema to the current Person Template. Legacy fields removed: person_name, birth_place (a four-value list), death_year, death_cause, alma_mater, allegiance, and the flat occupation and known_for lists. The type value was stored as '"person"' with embedded quotes, which broke type matching in every Dataview query in the vault; corrected to person.
- Legacy birth_place listed [[Duolij]], [[Postia]], [[Susia]], and [[Dripstanian Empire]] together. [[Susia]] removed: Paroska was born in 1796 and Susia did not exist until 1954. The style guide names this specific error.
- Legacy alma_mater [[Susian Naval Academy]] moved into the education block with degree and year blank, matching the handling in [[Kaichet Satratonie]].
- Legacy allegiance values split into two military_service entries, imperial then republican. Branch Navy and rank Admiral are drawn from the admiral title used in [[Jartes II]], the [[Susia]] country article, and [[Cericeiro honor code]]. Service dates are not established.
- The article had no body before this pass. All body content is summarized from Tier 1 articles: [[Cericeiro honor code]] for the family standing, the affair, the duel challenges, and Satratonie's motive; [[Jartes II]] and the [[Susia]] country article for the general government, the defection, and the execution; [[Jaffnidia]] for the fort and the museum; this article's own legacy frontmatter for birth, rank, and the treason conviction.
- INFERENCE: death city, state, and country. [[Jartes II]] places imperial troops in [[Neoveli]] in 1844 and the executions immediately after, but no article states Paroska's place of execution outright. Set to Neoveli to match [[Armadesh Versij]]; correct both together if wrong.
- SESSION CANON, confirmed: Paroska never answered the duel challenges. [[Cericeiro honor code]] establishes that Satratonie sought the duel repeatedly, that formal dueling ran between social equals, and that Satratonie was not accepted as one; that the challenges went unanswered follows, and was confirmed this session. Belongs primarily to a [[Liberal Revolts]] article once written.
- INFERENCE: ethnicity West Gaiyanese and religion [[Reformed Armotism]]. Neither is established for Paroska. Both are the values carried by his contemporaries [[Armadesh Versij]] and [[Kaichet Satratonie]] and are set here for consistency. Both are Required or Recommended fields, so leaving them blank would hold the article at stub.
- INFERENCE: political_alignment Republicanism, from his membership in the republic's general government. No article attributes a political position to him.
- charged_year and verdict_year set to 1844, matching the treatment in [[Armadesh Versij]].
- OPEN: the affair, the countess, and Satratonie's motive currently live only in [[Cericeiro honor code]], whose own flags cite a [[Liberal Revolts]] article that does not exist. This article is a second home for the same facts. One of the three needs to own them once [[Liberal Revolts]] is written.
- OPEN: [[Countess of Jafnidia]] has no article and canon gives her no personal name, only the title. [[Jaffnidia]] states that Satratonie first met her at the wall in that town.
- [[Susian Naval Academy]] is dated to 1803 in its own article but described there as a federal service academy, an identity that postdates 1954. Paroska would have trained there under whatever imperial institution preceded it. Name retained per the convention applied to [[Armadesh Versij]]; imperial-era institution names are unestablished vault-wide.
- No Personal Life section. The affair with the Countess of Jafnidia is directly politically relevant, being the stated cause of the defection that ended the revolt, and is treated in the Liberal Revolts section per the style guide.
- Main article links pointing to pages that do not exist: [[Liberal Revolts]], [[Federated Provinces of Galil]], [[Countess of Jafnidia]], [[Versij Day]], [[Versijian Square]].
- [[Kaichet Satratonie]] still carries no offices entry for the general government, no military_service values, and a wikilinked ethnicity that the person template reference specifies as plain text. He is the third member of this body and should be brought into line next.
- [[CHRONOLOGY]] holds nothing for 1840 to 1844 and no birth or death entry for Paroska.

%%
