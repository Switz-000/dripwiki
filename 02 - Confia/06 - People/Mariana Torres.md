---
# ── IDENTITY ────────────────────────────────────────────────────────────────
type: person
native_name: Mariana Marianowina Turreš Ljeranna
lusitanized_name: Mariana Torres
aliases:
  - Torres
summary: 

# ── DEMOGRAPHICS ────────────────────────────────────────────────────────────
sex: female
ethnicity: "[[East Gaiyanese]]"
religion: "[[Armotist Church of the Confian Synod]]"
citizenship:
  - "[[Dripstanian Empire|Dripstanian (revoked)]]"
  - "[[Confia|Confian]]"
nationality:
  - "[[Dripstanian Empire|Dripstanian]]"
  - "[[Confia|Confian]]"
enhanced: false

# ── BIRTH AND DEATH ────────────────────────────────────────────────────────────────
birth:
  year: 1896
  city: "[[Carlotopolis]]"
  state: "[[Karlotopol']]"
  country: "[[Dripstanian Empire]]"

death:
  year: 1988
  city: "[[Carlotopolis]]"
  state: "[[Karlotopol']]"
  country: "[[Confia]]"
  cause: "[[Tarlanna's disease]]"

# ── FAMILY ──────────────────────────────────────────────────────────────────
spouse: 
children_count:         # use um NÚMERO

# ── EDUCATION ───────────────────────────────────────────────────────────────
education:
  - degree: Civil Engineering
    institution: Polytechnic Institute of Koštanože
    year: 1920
  - 

# ── CAREER ──────────────────────────────────────────────────────────────────
occupation:
  - title:
    start_year:
    end_year:

military_service:
  - allegiance:
    branch: 
    rank:
    start_year:
    end_year:
    conflicts:
      -
    notes: null

political_alignment:
  - "[[Syndicalism#Revised Syndicalism|Six Points of Torres]]"

party: "[[Movement for Syndicalist Renovation|Movement for Syndicalist Renovation (1980-)]]"        # principal partido, ou partido atual
parties:                                        # partidos que ela ja pertenceu
  - "[[Movement for Syndicalist Renovation|Movement for Syndicalist Renovation (1980-)]]"
  - "[[Progressive Republican Party|Progressive Republican Party (1954-1980)]]"

organizations:
  -

# ── OFFICES ─────────────────────────────────────────────────────────────────
offices:
  - title: President of the Confian Nation
    employer: "[[Confian National Government]]"
    start_year: 1986
    end_year: 1987
    appointer: "[[1986 Confian presidential election]]"
    parties:
      - "[[Movement for Syndicalist Renovation]]"
    notes: 
  - title: President of the Confian Nation
    employer: "[[Confian National Government]]"
    start_year: 1978
    end_year: 1986
    appointer: "[[1978 Confian presidential election]]"
    parties:
      - "[[Movement for Syndicalist Renovation]]"
    notes: Left LjWP to found DSN in 1980
  - title: Leader of the Confian Nation
    employer: "[[Confian National Government]]"
    start_year: 1977
    end_year: 1977
    appointer: "[[Bayonet Revolution]]"
    parties:
      - "[[Progressive Republican Party]]"
    notes: Interim

# ── WRITTEN WORKS ───────────────────────────────────────────────────────────
written_works:
  - title: On fighting in the streets
    publication_year: 1979
    genre: Memoir
    notes: 


# ── CRIMINAL RECORD ─────────────────────────────────────────────────────────
criminal_charges:
  - charge: Treason
    counts: one
    charged_year: 
    plea: 
    verdict: guilty
    verdict_year: 
    sentence: Unlimited hard labour
    served: No, self-exile for 19 years then overthrew government
    in_absentia: true
    notes: Signed own amnesty

# ── ASSOCIATIONS & LEGACY ───────────────────────────────────────────────────
known_for:
  - item: "[[Bayonet Revolution]]"
    notes:
  - item: "[[Carlotopolis Uprising]]"
    notes:

awards:
  - title:
    awarded_year:
    posthumous: false
    granted_by:
    country:
    notes:

era:
  -

# ── META ────────────────────────────────────────────────────────────────────
tags:
  -

meta:
  stub: true
  verified: false
  image: null
---
**Mariana Marianowina Turreš Ljeranna** (Racpalian Alphabet: Марьана Марьановіна Турреш Љеранна) was a Confian civil engineer, revolutionary guerilla fighter and politician that served as the 2nd President of the [[Confia|United Syndicates of Confia]].



### Provisional Leader of Confia (1977)

As leader of the RBWIZ, Mariana Torres was installed as the interim leader, temporarily occupying the role of Leader of the Nation, created by the [[Presidential Empowerment Amendment]]. The Torres Government set out a transition timeline and new elections for late 1977, with the next government taking power in 1978. The 1956 Confian syndicalist constitution would be mostly restored to its original state by then, therefore removing the [[Presidential Empowerment Amendment]] and other constitutional measures taken by Paulowić. The only change from the original text was the abrogation of Article 3, which strictly defined cooperative and state ownership as the only legitimate forms of property.
 
Economically, the transition government carried out a series of hasty privatizations to pay off war debts and in an unsuccessful attempt to dismantle the ragged-sleeve power structure.
